...
{
	/* Currently, WinKVM support only IA32, so return value is always 0 */
    return 0;
}
...
int _cdecl vcpu_printf(struct kvm_vcpu *vcpu, const char *s, ...)
{
	va_list argList;
...
int _cdecl vm_entry_test_guest(struct kvm_vcpu *vcpu)  
{
	unsigned long cr0;
	unsigned long cr4;
	unsigned long cr3;
...
int _cdecl vm_entry_test_host(struct kvm_vcpu *vcpu)
{
	int r = 0;
...
int _cdecl vm_entry_test(struct kvm_vcpu *vcpu)
{
	int rg, rh;
